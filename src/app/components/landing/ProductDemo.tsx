...
const simulationData = [
  {
    ...
    steps: [
      {
        label: "1. Situation & Context",
        desc: "Redesigned the Acme Corp e-commerce platform dashboard.",
      },
      {
        label: "2. Complication & Challenge",
        desc: "Page load latency spiked by 40%, dropping customer conversion.",
      },
      {
        label: "3. Action Executed",
        desc: "Implemented route-based code splitting and component memoization.",
      },
      {
        label: "4. Quantified Outcome",
        desc: "Cut bundle sizes by 45% and recovered initial load drop.",
      },
      {
        label: "5. Alignment Connection",
        desc: "Demonstrates core performance optimization skills from the JD.",
      },
    ],
  },
  {
    question:
      "How do you handle scaling a service when traffic spikes unexpectedly?",
    resume: "Real-time chat notification service (Node.js, Redis, WebSockets).",
    skills: ["Redis", "Node.js", "WebSockets", "System Design"],
    framework: "CAR",
    steps: [
      {
        label: "1. Context & Scale",
        desc: "High-throughput real-time messaging pipeline handling 50k req/sec.",
      },
      {
        label: "2. Complication & Challenge",
        desc: "Redis memory saturation during daily peak hours crashed workers.",
      },
      {
        label: "3. Action Executed",
        desc: "Configured pub/sub clustering and optimized payload structures.",
      },
      {
        label: "4. Quantified Outcome",
        desc: "Achieved 99.99% availability during traffic spikes, cutting latency by 35%.",
      },
      {
        label: "5. Alignment Connection",
        desc: "Highlights distributed caching and backend scalability experience.",
      },
    ],
  },
  {
    question:
      "Describe a time when you had a disagreement with a product manager.",
    ...
    steps: [
      {
        label: "1. Situation & Context",
        desc: "Disagreement on adding a heavy feature right before a release.",
      },
      {
        label: "2. Complication & Conflict",
        desc: "PM wanted to push feature immediately; engineering feared bugs.",
      },
      {
        label: "3. Action Executed",
        desc: "Facilitated a quick RICE prioritization risk assessment workshop.",
      },
      {
        label: "4. Quantified Outcome",
        desc: "Agreed to release feature in Phase 2, maintaining timeline stability.",
      },
      {
        label: "5. Alignment Connection",
        desc: "Evidences cross-functional collaboration and stakeholder management.",
      },
    ],
  },
  {
    ...
    steps: [
      {
        label: "1. Situation & Context",
        desc: "Optimizing a heavy analytics dashboard load speed.",
      },
      {
        label: "2. Complication & Diagnostic",
        desc: "Initial Lighthouse score was 48 due to unoptimized assets.",
      },
      {
        label: "3. Action Executed",
        desc: "Replaced heavy packages, enabled SSR caching, and lazy-loaded modules.",
      },
      {
        label: "4. Quantified Outcome",
        desc: "Boosted Lighthouse score to 92 and halved Largest Contentful Paint (LCP).",
      },
      {
        label: "5. Alignment Connection",
        desc: "Demonstrates deep technical knowledge of Next.js and page speed mechanics.",
      },
    ],
  },
  {
    ...
    steps: [
      {
        label: "1. Situation & Context",
        desc: "Pushed a database configuration error to production.",
      },
      {
        label: "2. Complication & Incident",
        desc: "Caused a 5-minute checkout outage for 4% of active users.",
      },
      {
        label: "3. Action Executed",
        desc: "Immediately rolled back deployment, resolved config, and wrote a post-mortem.",
      },
      {
        label: "4. Quantified Outcome",
        desc: "Created automated pre-deployment check scripts to prevent future errors.",
      },
      {
        label: "5. Alignment Connection",
        desc: "Illustrates professional accountability and strong problem-solving under pressure.",
      },
    ],
  },
];
...
export default function ProductDemo() {
  ...
  return (
    ...
    >
      <div className="orb orb-peach w-125 h-125 top-1/4 -left-40 animate-float-orb opacity-50" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
        <ScrollReveal>
          <div className="text-center mb-6">
            ...
            <p className="text-(--text-muted) text-base mt-3 max-w-xl mx-auto">
              Select an interview question below to watch the interactive
              simulator analyze the audio, match context, and stream response
              outlines.
            </p>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal className="w-full">
          {/* Main App Window */}
          <div className="w-full bg-white border border-(--border-light) rounded-md shadow-lg overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50/50 shrink-0 select-none">
              ...
              <span className="text-[11px] font-mono text-slate-400 font-bold">
                stealth_copilot_session.exe •{" "}
                {isAutoplay ? "Auto-Align Feed" : "Interactive Feed"} (Q
                {currentIdx + 1}/{simulationData.length})
              </span>

              {/* Balancer spacer */}
              <div className="w-13 h-3 hidden sm:block" />
            </div>
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 bg-white">
              ...
              <div className="bg-slate-50/50 border border-slate-100 rounded-md p-5 flex flex-col min-h-90 justify-between">
                {/* Column Header */}
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`w-2 h-2 rounded-full ${isPlaying && !isQuestionDone ? "bg-red-500 animate-pulse" : "bg-(--accent)"}`}
                    />
                ...
                <div className="flex-1 flex flex-col justify-between py-4 gap-6">
                  ...
                  <div className="flex items-end justify-center gap-0.75 h-14 shrink-0 relative select-none">
                    {isPlaying && !isQuestionDone ? (
                      [
                        4, 12, 22, 14, 8, 26, 38, 30, 16, 34, 44, 24, 12, 18, 6,
                      ].map((h, i) => (
                        <span
                          key={i}
                  ...
                  <div className="flex-1 flex flex-col justify-start">
                    ...
                    <p className="text-[12px] font-mono text-slate-700 leading-relaxed text-left">
                      {questionText ||
                        (isPlaying ? "..." : currentData.question)}
                      {isPlaying && !isQuestionDone && (
              ...
              <div className="bg-slate-50/50 border border-slate-100 rounded-md p-5 flex flex-col min-h-90 justify-between">
                ...
                <div className="flex-1 flex flex-col gap-3 py-4">
                  <div className="bg-white rounded-md p-3.5 flex items-start gap-2.5 shadow-xs border-0">
                  ...
                  <div className="bg-white rounded-md p-3.5 flex items-start gap-2.5 shadow-xs border-0">
                    ...
                    <div>
                      <span className="text-[10px] font-extrabold text-slate-700 block mb-1.5">
                        JD Keyword Match
                      </span>
                      {isPlaying && !isQuestionDone ? (
              ...
              <div className="bg-slate-50/50 border border-slate-100 rounded-md p-5 flex flex-col min-h-90 justify-between relative overflow-hidden">
                {/* Column Header */}
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                ...
                <div className="flex flex-col gap-3 min-h-55 py-4">
                  {isLoading && (
                  ...
                  {!isLoading &&
                    currentData.steps.map((step, idx) => {
                      ...
                      const isProcessing =
                        idx === answersVisible && !isLoading && isPlaying;
                      return (
                        <div
                          key={idx}
                ...
                <div className="text-[9px] font-mono text-slate-400 pt-3 border-t border-slate-100 flex items-center justify-between font-bold">
                  ...
                  <span>
                    Steps: {answersVisible}/{currentData.steps.length}
                  </span>
                </div>
              </div>
            </div>
