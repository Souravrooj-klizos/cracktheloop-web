...
import { Globe, Download, ArrowRight, Check, X } from "lucide-react";
...
export default function PlatformPicker() {
  ...
  const platforms = [
    {
      ...
      description:
        "Launch instantly in your browser without downloading anything. Evaluates your coding and system design skills on the fly.",
      features: [
    ...
    },
    {
      ...
      description:
        "Our flagship desktop application. Deep system-level security integration delivers 100% invisible stealth overlay and local-first privacy.",
      features: [
    ...
    },
    {
      ...
      color: "bg-slate-100 text-slate-700 border border-slate-200",
      description:
        "Fully optimized for macOS. Natively compiled for Apple M1/M2/M3 chips and Intel processors with modern Metal graphics rendering.",
      features: [
  ...
  return (
    <section
      id="platform-picker"
      className="section-mist relative py-14 md:py-16 overflow-hidden"
    >
      {/* Decorative Orbs */}
      <div className="orb orb-peach w-125 h-125 -top-20 -left-20 opacity-40 pointer-events-none animate-float-orb" />
      <div className="orb orb-slate w-100 h-100 -bottom-20 -right-20 opacity-30 pointer-events-none animate-float-orb-slow" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <ScrollReveal>
          <div className="text-center mb-10 flex flex-col items-center">
            ...
            >
              One Product.{" "}
              <span className="text-gradient-coral">Every Platform</span> You
              Use.
            </h2>
            <p className="text-(--text-muted) text-base max-w-xl mt-3 leading-relaxed">
              CrackTheLoop works wherever you do. Choose the web copilot for
              instant access, or download the desktop app for ironclad stealth
              privacy.
            </p>
          </div>
        </ScrollReveal>
            ...
            {platforms.map((platform) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white/85 backdrop-blur-md border rounded-xl p-6 md:p-8 flex flex-col justify-between shadow-xs transition-all duration-300 relative cursor-pointer select-none group ${
                    isFeatured
                ...
                >
                  <div>
                    <div className="flex justify-between items-start gap-4 mb-6">
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center bg-slate-50 border border-slate-200 text-slate-700 transition-colors duration-300 ${isFeatured ? "border-(--accent) text-(--accent) bg-(--accent-soft)" : "group-hover:border-(--accent)/40 group-hover:text-(--accent)"}`}
                      >
                        <Icon className="w-5 h-5" />
                      </div>
                      <span
                        className={`text-[9px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full ${isFeatured ? "bg-(--accent-soft) text-(--accent) border border-(--accent)/20" : "bg-slate-100 text-slate-500 border border-slate-200"}`}
                      >
                        {platform.tag}
                      </span>
                    </div>
                    ...
                    <h3
                      className="text-lg font-bold text-(--text-primary) group-hover:text-(--accent) transition-all duration-300"
                      style={{ fontFamily: "var(--font-display)" }}
                    >
                      {platform.name}
                    </h3>

                    <p className="text-xs text-(--text-muted) mt-3 leading-relaxed min-h-12.5">
                  ...
                  <div>
                    <Link
                      ...
                      onClick={(e) =>
                        handleCtaClick(e, platform.id, platform.ctaLink)
                      }
                    ...
                    {platform.isDownload && (
                      <p className="text-[10px] text-(--text-muted) text-center mt-3 font-medium">
                        {platform.id === "mac"
                          ? "Roadmapped for native Q3 2026 release."
              ...
              <>
                ...
                <div className="flex flex-col gap-2 text-left">
                  ...
                  <p className="text-xs text-(--text-muted) leading-relaxed">
                    Our native macOS client with CoreAudio and Metal rendering
                    is roadmapped for release in <strong>Q3 2026</strong>. Enter
                    your email to join our early beta testing circle.
                  </p>
                </div>
              ...
              <div className="flex flex-col items-center text-center gap-4 py-4">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-xs text-(--text-muted) leading-relaxed max-w-xs">
                    We'll email you at <strong>{macEmail}</strong> as soon as
                    the first beta builds of the macOS app become available.
                  </p>
                </div>
                <button
                  onClick={() => {
