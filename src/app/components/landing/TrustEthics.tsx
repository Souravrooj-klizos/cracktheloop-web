...
export default function TrustEthics() {
  return (
    ...
    >
      {/* Subtle orbs */}
      <div className="orb orb-slate w-87.5 h-87.5 -top-20 -left-20 animate-float-orb opacity-40" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
        <ScrollReveal>
          <div className="text-center mb-16">
            ...
            >
              Built for{" "}
              <span className="text-gradient-coral">Responsible Use</span>
            </h2>
            <p className="text-(--text-muted-dark) text-base mt-3 max-w-2xl mx-auto">
              CrackTheLoop is designed to support communication confidence,
              preparation structuring, and permitted interview assistance.
            </p>
          </div>
        </ScrollReveal>
        ...
        >
          {trustPoints.map((point, i) => (
            <StaggerItem key={i}>
              <div className="glass-card-light rounded-md p-5 h-full flex flex-col items-start gap-4 border border-(--border-light) bg-white/70 backdrop-blur-md hover:-translate-y-1.5 transition-all duration-300">
                <div className="w-10 h-10 rounded-md bg-white flex items-center justify-center border border-(--border-light) shadow-xs">
                  {point.icon}
                </div>
                <h3 className="text-base font-bold text-(--text-primary-dark)">
                  {point.title}
                </h3>
