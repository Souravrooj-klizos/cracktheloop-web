...
const comparisonPoints = [
  ...
  },
  {
    ...
    cracktheloop: true,
    detail:
      "Final Round AI gives generic, surface-level suggestions. CrackTheLoop tailors to title and seniority.",
  },
  {
    ...
    cracktheloop: true,
    detail:
      "No complex prompting - answers appear automatically as questions are asked.",
  },
  {
    ...
    cracktheloop: true,
    detail:
      "Free tier includes 50 credits - evaluate the platform first, no card needed.",
  },
  {
    ...
    cracktheloop: true,
    detail:
      "Real-time response suggestions during your sessions - not just pre-interview prep.",
  },
  {
    ...
    cracktheloop: true,
    detail:
      "Win32 Display Affinity makes the overlay invisible to all screen-sharing software.",
  },
  {
    ...
    cracktheloop: true,
    detail:
      "Automatically maps questions to behavioral frameworks with your experience.",
  },
  {
    ...
    cracktheloop: true,
    detail:
      "Suggests talking points - not scripts - so answers sound like you.",
  },
];
...
export default function Comparison() {
  ...
  useEffect(() => {
    ...
    fetch("https://freeipapi.com/api/json")
      ...
      .catch((err) => {
        console.warn(
          "FreeIPAPI failed, running fallback timezone detection:",
          err,
        );
        runTimezoneFallback();
      });
  ...
  const rows =
    currency === "INR"
      ? [
          {
            label: "Human Mock Interviewer",
            cost: "₹12,000–₹25,000 / hr",
            muted: true,
          },
          {
            label: "Interview Prep Course",
            cost: "₹80,000–₹4,00,000",
            muted: true,
          },
          { label: "Career Coach (6 weeks)", cost: "₹2,50,000+", muted: true },
          {
            label: "CrackTheLoop Pro Pass",
            cost: "₹1499 / one-time",
            highlight: true,
          },
        ]
  ...
  return (
    ...
    >
      {/* Background elements */}
      <div className="orb orb-peach w-125 h-125 -top-40 -left-40 animate-float-orb" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
          ...
          <div className="text-center mb-16">
            ...
            >
              ...
              <span className="text-gradient-coral">
                - CrackTheLoop vs the Rest
              </span>
            </h2>
            <p className="text-(--text-muted) text-base mt-3 max-w-xl mx-auto">
              Why a dedicated AI interview helper that works during live
              interviews outperforms ChatGPT, Final Round AI, and generic prep
              tools - every time.
            </p>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal className="w-full">
          ...
          <div className="overflow-hidden rounded-xl border border-(--border-light) shadow-xs bg-white/85 backdrop-blur-md w-full">
            <div className="overflow-x-auto w-full">
              <table className="w-full min-w-145 text-sm text-left">
                <thead>
                ...
                <tbody className="divide-y divide-(--border-light)">
                  {comparisonPoints.map((point, i) => (
                    <tr
                      key={i}
                      className="hover:bg-(--bg-mist) transition-colors"
                    >
        ...
        <ScrollReveal className="mt-10">
          <div className="bg-white/80 backdrop-blur-md border border-(--border-light) rounded-xl p-6 md:p-8 flex flex-col md:flex-row gap-8 items-center shadow-xs">
            <div className="flex-1">
              <span className="text-[10px] font-mono font-bold tracking-wider uppercase text-(--accent) mb-3 block">
                Value Comparison
              </span>
              <h3
                className="text-xl md:text-2xl font-extrabold text-(--text-primary) leading-snug mb-1"
                style={{ fontFamily: "var(--font-display)" }}
              >
                Maximize your interview preparation.
                <br />
                <span className="text-gradient-coral">
                  High-quality practice tools at a fraction of the cost.
                </span>
              </h3>
              <p className="text-sm text-(--text-muted) mt-2 max-w-md leading-relaxed">
                Compare the real cost of alternatives. Practice with real-time
                feedback and structured answers to build interview confidence
                efficiently.
              </p>
            </div>
            <div className="shrink-0 w-full md:w-auto">
              <div className="overflow-hidden rounded-lg border border-(--border-light) text-sm w-full md:w-80">
                {rows.map((row, i) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between px-4 py-3 border-b border-(--border-light) last:border-0 ${
                      row.highlight
                  ...
                  >
                    <span
                      className={`text-xs font-medium ${row.highlight ? "text-(--accent) font-bold" : "text-(--text-secondary)"}`}
                    >
                      {row.label}
                    </span>
                    <span
                      className={`text-xs font-bold whitespace-nowrap ml-4 ${row.highlight ? "text-(--accent)" : "text-(--text-muted)"}`}
                    >
                      {row.cost}
                    </span>
                  </div>
            ...
            {[
              {
                ...
                desc: "Never shared, logged, or used for model training. Stays 100% on your device.",
              },
              {
                title: "Invisible to Zoom & Teams",
                desc: "Win32 Display Affinity API ensures the overlay is never captured in screen recordings.",
              },
              {
                title: "No Virtual Audio Drivers",
                desc: "Uses WASAPI system loopback - nothing unusual appears in your device list.",
              },
              {
                title: "User-Controlled Sessions",
                desc: "Copilot only listens when you explicitly start a session. Zero passive monitoring.",
              },
            ].map((tp, i) => (
              <StaggerItem key={i}>
                <div className="rounded-lg border border-(--border-light) bg-white/70 backdrop-blur-md p-5 flex flex-col gap-2 hover:border-(--accent)/20 hover:-translate-y-0.5 transition-all duration-300 h-full">
