...
import { Menu, X, ChevronDown, ArrowRight, Download, Gift } from "lucide-react";
...
export default function Navbar() {
  ...
  const dismissBanner = () => {
    setBannerOpen(false);
    document.cookie =
      "ctl_banner_dismissed=true; path=/; max-age=604800; SameSite=Lax";
  };
            ...
            <div className="flex-1 flex items-center justify-center gap-2 sm:gap-3 text-xs md:text-sm font-semibold text-slate-800">
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <span className="hidden sm:inline font-medium text-slate-600">
                  <strong>Launch Offer:</strong> First 100 signups get{" "}
                  <strong className="text-slate-900">50 free credits</strong> +{" "}
                  <strong className="text-slate-900">20% purchase bonus</strong>{" "}
                  on referral. Only{" "}
      ...
      >
        <div className="px-6 py-2 md:py-1 flex justify-between items-center w-full">
          ...
          >
            ...
            />
            <span
              className="font-bold tracking-tight text-xl md:text-2xl text-(--text-primary)"
        ...
        <div className="absolute inset-0 rounded-full overflow-hidden pointer-events-none">
          <motion.div
            className="absolute bottom-0 left-0 right-0 h-0.75 bg-(--accent) origin-left z-20"
            style={{ scaleX }}
          />
        </div>
      </motion.header>
