...
export default function Sidebar({ user, onLogout }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex md:w-64 bg-white border-r border-[var(--border-light)] flex-col p-6 relative z-30 justify-between select-none shadow-xs h-full shrink-0">
        <div className="flex flex-col gap-8">
          {/* Logo */}
          <a href="/" className="flex items-center gap-2.5 hover:opacity-90 transition px-2">
            <img src="/logo.svg" className="h-9 w-9 rounded-lg select-none border border-[var(--border-light)]" alt="CrackTheLoop Logo Icon" />
              ...
              return (
                <a
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition duration-200 group border ${
                    isActive 
                      ? "bg-[var(--accent-soft)] text-[var(--accent)] border-[var(--accent)]/15 shadow-xs" 
                      : "text-slate-500 border-transparent hover:text-slate-800 hover:bg-slate-50"
                  }`}
                >
                  <Icon className={`w-4 h-4 transition ${isActive ? "text-[var(--accent)]" : "text-slate-400 group-hover:text-slate-600"}`} />
                  {item.name}
                </a>
              );
            })}
      ...
      {/* Mobile Top Header */}
      <div className="md:hidden w-full bg-white border-b border-[var(--border-light)] px-6 py-4 flex justify-between items-center relative z-40 select-none shadow-xs shrink-0">
        <a href="/" className="flex items-center gap-2.5 hover:opacity-90 transition">
          <img src="/logo.svg" className="h-8 w-8 rounded-lg select-none border border-[var(--border-light)]" alt="CrackTheLoop Logo Icon" />
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-x-0 top-[57px] bottom-0 bg-white/95 backdrop-blur-md z-30 flex flex-col p-6 justify-between select-none animate-fade-in border-b border-[var(--border-light)] shadow-md">
          ...
          <div className="flex flex-col gap-4 border-t border-[var(--border-light)] pt-4">
            ...
            <div className="flex flex-col gap-2 font-semibold">
              ...
              >
                <Home className="w-3.5 h-3.5 text-slate-450" />
                Website Home
              </a>
              <a 
                href="/copilot"
