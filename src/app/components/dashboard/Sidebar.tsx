    ...
    <>
      ...
      <aside className="hidden md:flex md:w-64 bg-white border-r border-(--border-light) flex-col p-6 relative z-30 justify-between select-none shadow-xs h-full shrink-0">
        <div className="flex flex-col gap-6">
          ...
          <Link
            href="/"
            className="flex items-center gap-2.5 hover:opacity-90 transition px-2"
          >
            <img
              src="/logo.png"
            ...
            />
            <span
              className="font-bold tracking-tight text-xl text-slate-800"
              style={{ fontFamily: "var(--font-display)" }}
            >
              Crack
              <span className="text-gradient-coral font-black">TheLoop</span>
            </span>
          </Link>

          {/* Navigation links */}
          <nav className="flex flex-col gap-2">
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest px-3 mb-2">
              Portal Pages
            </span>
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    className={`w-4 h-4 transition ${isActive ? "text-(--accent)" : "text-slate-400"}`}
                  />
                  {item.name}
                </Link>
              );
        ...
        <div className="flex flex-col gap-3 border-t border-(--border-light) pt-3">
          ...
          <div className="mx-1 bg-linear-to-br from-indigo-50 to-red-50/50 border border-indigo-100/50 p-3.5 rounded-xl flex flex-col gap-2.5 relative overflow-hidden group shadow-xs select-none">
            ...
            <Link
              href="/pricing"
              className="w-full py-2 bg-accent hover:bg-accent-bright hover:brightness-110 text-white rounded-lg text-center font-bold text-[10px] uppercase tracking-wider transition active:scale-95 shadow-sm shadow-accent/10 hover:shadow-accent/20"
            >
              Get Credits
            </Link>
          </div>
      ...
      <div className="md:hidden w-full bg-white border-b border-(--border-light) px-6 py-4 flex justify-between items-center relative z-40 select-none shadow-xs shrink-0">
        ...
        >
          ...
          />
          <span
            className="font-bold tracking-tight text-lg text-slate-800"
            style={{ fontFamily: "var(--font-display)" }}
          >
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-x-0 top-14.25 bottom-0 bg-white/95 backdrop-blur-md z-30 flex flex-col p-6 justify-between select-none animate-fade-in border-b border-(--border-light) shadow-md">
          <div className="flex flex-col gap-6">
            <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest px-2">
              Navigation
            </span>
            <nav className="flex flex-col gap-2">
              {navItems.map((item) => {
          ...
          <div className="flex flex-col gap-4 border-t border-(--border-light) pt-4">
            ...
            <div className="mx-2 bg-linear-to-br from-indigo-50 to-red-50/50 border border-indigo-100/50 p-3 rounded-lg flex items-center justify-between gap-3 shadow-xs select-none">
              ...
              <Link
                href="/pricing"
                onClick={() => setMobileMenuOpen(false)}
                className="px-4 py-2 bg-accent hover:bg-accent-bright hover:brightness-110 text-white rounded-lg font-bold text-[10px] uppercase tracking-wider shrink-0 transition"
              >
                Get Credits
              </Link>
            </div>
            ...
            <div className="px-2">
              ...
              <span className="text-xs font-bold text-slate-600 truncate mt-1 block">
                {user?.email}
              </span>
            </div>
