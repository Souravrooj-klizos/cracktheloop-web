...
export default function PricingPage() {
      ...
      .then((data) => {
        if (data.success) {
          ...
          if (data.settings) {
            ...
            setTrialExpiryDays(
              data.settings.trial_expiry_days ??
                data.settings.trial_expiration_days ??
                -1,
            );
          }
        }
  ...
  const getPlanData = (
    ...
    return {
      ...
      features:
        matched && matched.features?.length > 0
          ? matched.features
          : defaultFeatures,
  ...
  function handlePlanSelect(planName: string) {
    router.push(
      `/select-plan?plan=${encodeURIComponent(planName)}&currency=${currency}`,
    );
  }
  ...
  return (
    <div className="min-h-screen bg-(--bg-mist) text-(--text-primary) flex flex-col pt-30">
      {/* Background orbs */}
      <div className="orb orb-peach w-150 h-150 -top-40 left-1/4 animate-float-orb opacity-40 pointer-events-none" />
      <div className="orb orb-slate w-100 h-100 bottom-0 -right-20 animate-float-orb-slow opacity-40 pointer-events-none" />
      ...
      <section className="w-full max-w-7xl mx-auto px-6 pt-12 text-center flex flex-col items-center gap-4 relative z-20 select-none">
        <h1
          className="text-4xl md:text-5xl font-black tracking-tight leading-tight text-slate-800"
          style={{ fontFamily: "var(--font-display)" }}
        >
          One-Time Passes for{" "}
          <span className="text-gradient-coral">Unlimited Confidence</span>
        </h1>
        <p className="text-(--text-muted) text-sm md:text-base max-w-xl leading-relaxed">
          Start your free trial in seconds. Buy a one-time pass to load fuel
          credits — credits never expire and activate instantly.
        </p>
      </section>
      ...
      <section className="w-full max-w-7xl mx-auto px-6 pt-12 relative z-20">
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${plans.length} gap-8 items-stretch max-w-5xl mx-auto`}
        >
          {plans.map((plan) => {
            ...
            return (
              <div
                ...
                className={`backdrop-blur-md rounded-xl p-6 md:p-8 flex flex-col justify-between transition-all duration-300 min-h-105 relative ${
                  isPro
                  ...
                  <ul className="text-xs text-slate-655 flex flex-col gap-3.5 border-t border-slate-100 pt-6 mt-6">
                    {plan.features?.map((feat: string, idx: number) => {
                      ...
                      return (
                        <li
                          key={idx}
                          className={`flex items-center gap-2 ${isFirst ? "font-bold text-(--accent)" : ""}`}
                        >
                          <Check
                            className={`w-4 h-4 ${isFirst || isPro ? "text-(--accent)" : "text-slate-400"}`}
                          />
                          {feat}
                        </li>
                      );
                ...
                <div className="mt-8">
                  <button
                    ...
                    className={`w-full text-center justify-center cursor-pointer py-3.5 px-6 text-xs uppercase tracking-wider ${
                      isPro
                        ? "btn-primary shadow-md shadow-accent/20"
                        : "btn-ghost-dark font-bold"
                    }`}
                  >
                    {isFree
                      ? "Start Trial"
                      : `Upgrade to ${plan.name.replace(" Pass", "").replace(" Plan", "")}`}
                  </button>
                </div>
              </div>
            );
      ...
      <section
        className="w-full max-w-7xl mx-auto px-6 pt-20 pb-8 relative z-20 select-none"
        id="referrals-section"
      >
        <div className="bg-white/85 backdrop-blur-md border border-(--border-light) rounded-xl p-6 md:p-8 flex flex-col gap-6 shadow-xs">
          <div className="flex flex-col gap-1.5 border-b border-slate-100 pb-4">
            ...
            <p className="text-sm md:text-base text-slate-500 leading-relaxed max-w-xl font-semibold">
              Share CrackTheLoop with friends and colleagues - both of you get
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-50/65 border border-slate-200/80 rounded-xl p-5 flex flex-col gap-4">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-full bg-(--accent-soft) flex items-center justify-center border border-(--accent)/15">
                  <UserPlus className="w-4 h-4 text-(--accent)" />
                </div>
                <span className="text-xs md:text-sm font-black text-slate-800 uppercase tracking-widest">
                  You're Invited
                </span>
              </div>
              <p className="text-sm md:text-base text-slate-600 leading-relaxed font-semibold">
                When you sign up using a friend's referral link, you get{" "}
                <span className="text-(--accent) font-bold">
                  50 free credits
                </span>{" "}
                instantly on sign-up to start practicing mock interviews.
              </p>
            </div>
            ...
            <div className="bg-slate-50/65 border border-slate-200/80 rounded-xl p-5 flex flex-col gap-4">
              ...
              <p className="text-sm md:text-base text-slate-650 leading-relaxed font-semibold">
                When a referred friend signs up and activates their trial,{" "}
                <span className="text-(--accent) font-bold">
                  both of you get 50 free credits
                </span>{" "}
                instantly.
              </p>
            </div>
          ...
          <div className="flex flex-col items-center gap-2 border-t border-slate-100 pt-6 mt-2">
            <button
              onClick={() => {
                ...
                if (token) {
                  ...
                  router.push(
                    "/login?mode=signup&redirect=/dashboard/referrals",
                  );
                }
              }}
            ...
            <span className="text-[10px] text-slate-400 font-semibold italic">
              *Sign in or create a free account to access your personal referral
              link.
            </span>
          </div>
        </div>
