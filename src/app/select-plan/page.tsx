...
function SelectPlanContent() {
  ...
  useEffect(() => {
    ...
    if (!token || !userStr) {
      ...
      window.location.replace(
        `/login?mode=signup&plan=${encodeURIComponent(planName)}`,
      );
      return;
    }
    ...
    async function resolvePlanAndCheckout() {
      ...
      try {
        ...
        if (!plansRes.ok || !plansData.success) {
          throw new Error(
            plansData.error || "Failed to load plans from database",
          );
        }
        ...
        if (!dbPlan || !dbPlan.price_id) {
          throw new Error(
            `Plan "${planName}" is not currently available or active.`,
          );
        }
        ...
        setStatusText(
          `Preparing your secure checkout session for ${dbPlan.name}...`,
        );
        ...
        setErrorMsg(
          err.message || "An error occurred while connecting to Stripe.",
        );
        setLoading(false);
      }
    }
  ...
  if (errorMsg) {
    return (
      <div className="min-h-screen bg-(--bg-mist) flex flex-col justify-center items-center py-12 px-6">
        <div className="w-full max-w-md bg-white border border-(--border-light) rounded-xl p-6 shadow-md flex flex-col gap-5 items-center text-center">
  ...
  return (
    <div className="min-h-screen bg-(--bg-mist) flex flex-col justify-center items-center relative overflow-hidden py-12 px-6">
      {/* Background orbs */}
      <div className="orb orb-peach w-125 h-125 -top-20 -left-20 animate-float-orb opacity-40 pointer-events-none" />
      <div className="orb orb-slate w-100 h-100 -bottom-20 -right-20 animate-float-orb-slow opacity-40 pointer-events-none" />

      <div className="w-full max-w-md bg-white border border-(--border-light) rounded-xl p-8 shadow-md flex flex-col gap-6 items-center text-center relative z-10">
        ...
        <div className="flex items-center gap-1.5 text-[9px] font-black text-slate-400 bg-slate-50 border border-slate-100 px-3 py-1 rounded-full uppercase tracking-widest mt-2">
          <Lock className="w-3.5 h-3.5 text-emerald-500" /> Secure SSL Stripe
          Gateway
        </div>
      </div>
    </div>
