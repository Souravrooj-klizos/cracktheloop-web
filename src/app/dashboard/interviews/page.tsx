    ...
    <main className="flex-1 w-full px-6 md:px-10 py-5 md:py-6 flex flex-col gap-6 relative select-none">
      ...
      <section className="flex flex-col gap-2">
        ...
        <p className="text-xs text-slate-500 font-medium">
          Browse transcripts, technical grading metrics, and AI recommendations.
        </p>
      </section>
      ...
      <section className="bg-white border border-slate-200/60 rounded-xl p-5 md:p-6 flex flex-col gap-5 shadow-sm relative z-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 flex flex-col items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-red-50/50 border border-red-100 flex items-center justify-center text-accent shadow-sm mb-2">
              <Mic className="w-6 h-6 animate-pulse" />
            </div>
            {searchQuery ? (
              <p className="text-slate-500 text-xs font-bold">
                No interviews match your search term.
              </p>
            ) : (
              <>
                ...
                <p className="text-slate-500 text-xs max-w-sm leading-relaxed font-semibold mt-0.5">
                  Activate the Web Copilot HUD or Desktop app during your live
                  interview sessions, and click "Save Session" to compile your
                  transcripts here.
                </p>
                <Link
              ...
              <tbody className="divide-y divide-slate-100/80 font-semibold text-slate-600">
                {filteredInterviews.map((session) => {
                  ...
                  return (
                    <tr
                      key={session._id}
                      className="hover:bg-slate-50/70 transition-all duration-200 group"
                    >
                      ...
                      <td className="py-4.5 px-5">{scoreBadge}</td>
                      <td className="py-4.5 px-5 text-right">
                        <button
                          onClick={() =>
                            router.push(`/dashboard/interviews/${session._id}`)
                          }
